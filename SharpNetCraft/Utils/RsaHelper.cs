		...
		public static RSA DecodePublicKey(byte[] publicKeyBytes)
		{
			...
			try
			{
				...
				switch (shortValue)
				{
					...
					default:
						Logger.GetLogger().Warn($"PublicKey Decode Returning null!");
				...
				{
					Logger.GetLogger().Warn($"PublicKey Decode Returning null! (shortvalue 1)");
				...
				if (byteValue != 0x00)
				{
					Logger.GetLogger().Warn($"PublicKey Decode Returning null! (bytevalue)");
				...
				{
					Logger.GetLogger().Warn($"PublicKey Decode Returning null! (Shortvalue 2)");
			...
			{
				Logger.GetLogger().Warn($"PublicKey Decode Exception: {e}");
