					...
					while (!_cancelSource.IsCancellationRequested)
					{
						...
						if (msLeft < 16)
						{
							...
							if (!skipTicks)
							{
								...
								if (t < -5) Logger.GetLogger().Warn($"We overslept {t}ms in thread yield/sleep");
							}
							continue;
						}
						...
						{
							long t = nextStop - watch.ElapsedMilliseconds;
							if (t < -5) Logger.GetLogger().Warn($"We overslept {t}ms in thread sleep");
						}
					}
				}
				catch (Exception e)
				{
					Logger.GetLogger().Error("Timer crashed out with uncaught exception leaving it in an undisposed state. " + e.Message);
					throw;
				}
				finally
				{
